...
public class StevesRealisticSleepApi {
	/**
	 * @param level The level (aka dimension)
	 * @return The sleep progress of the level as a percentage.
	 * @since v1.8.4
	 */
	@SuppressWarnings("unused")
	public static float getSleepProgress(@NotNull Level level) {
		return ((float) (DAY_LENGTH - calculateTicksUntilAwake(getTimeOfDay(level))) / DAY_LENGTH) * 100f;
	}

	/**
	 * @param level The level (aka dimension)
	 * @return The time of day of the level in ticks, with the day count filtered out using a modulo operator. This means that when Minecraft returns the time of day of a level, the day count doesn't have any effect on the resulting time. Equal to <code>(int) level.getLevelData().getDayTime() % DAY_LENGTH</code>.
	 * @since v1.8.4
	 */
	public static int getTimeOfDay(@NotNull Level level) {
		return (int) level.getLevelData().getDayTime() % DAY_LENGTH;
	}

	/**
	 * @param level The level (aka dimension)
	 * @return Whether players are sleeping in this world (time is passing at an accelerated rate). The level will be casted to either a <code>ServerLevel</code> or <code>ClientLevel</code> depending on the side this method is called from. Prefer the sides' respective API methods instead.
	 */
	@SuppressWarnings("BooleanMethodIsAlwaysInverted")
	public static boolean isSleeping(@NotNull Level level) {
		if (level.isClientSide()) {
			return isSleeping((ClientLevel) level);
		} else {
			return isSleeping((ServerLevel) level);
		}
	}

	/**
	 * @param level The clientside level (aka dimension)
	...
	/**
	 * @param level The serverside level (aka dimension)
	 * @return Whether players are sleeping in this level (time is passing at an accelerated rate).
	 */
	 ...
	 */
	@SuppressWarnings("unused")
	public static int getConsecutiveSleepTicks(ServerLevel level) {
		return ((ServerWorldExtension) level).consecutiveSleepTicks;
	}
}
